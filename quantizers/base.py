...
class DiffusersQuantizer(ABC):
    """
    Abstract class of the HuggingFace quantizer. Supports for now quantizing HF custom_diffusers models for inference and/or
    quantization. This class is used only for custom_diffusers.models.modeling_utils.ModelMixin.from_pretrained and cannot be
    easily used outside the scope of that method yet.

    Attributes
        quantization_config (`custom_diffusers.quantizers.quantization_config.QuantizationConfigMixin`):
    ...
    def get_special_dtypes_update(self, model, torch_dtype: "torch.dtype") -> Dict[str, "torch.dtype"]:
        ...
        `_process_model_before_weight_loading`. `custom_diffusers` models don't have any `modules_to_not_convert` attributes
        yet but this can change soon in the future.

        Args:
            model (`~custom_diffusers.models.modeling_utils.ModelMixin`):
    ...
    def validate_environment(self, *args, **kwargs):
        ...
        `from_pretrained`. You need to define it for all future quantizers that are integrated with custom_diffusers. If no
