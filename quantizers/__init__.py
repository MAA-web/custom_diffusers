...
class PipelineQuantizationConfig:
    ...
    Args:
        quant_backend (`str`): Quantization backend to be used. When using this option, we assume that the backend
            is available to both `custom_diffusers` and `transformers`.
    ...
    def _validate_init_kwargs_in_backends(self):
        ...
        if init_kwargs_transformers != init_kwargs_diffusers:
            raise ValueError(
                "The signatures of the __init__ methods of the quantization config classes in `custom_diffusers` and `transformers` don't match. "
                f"Please provide a `quant_mapping` instead, in the {self.__class__.__name__} class. Refer to [the docs](https://huggingface.co/docs/custom_diffusers/main/en/quantization/overview#pipeline-level-quantization) to learn more about how "
        ...
        for module_name, config in quant_mapping.items():
            ...
            if available_transformers:
                raise ValueError(
                    ...
                    f"Available custom_diffusers configs: {available_diffusers}; "
    ...
    def _check_backend_availability(self, quant_backend: str):
        ...
        if (
            ...
            error_message += f"\nAvailable ones (custom_diffusers): {available_backends_diffusers}."
