...
def _map_component_types_to_config_dict(component_types):
    ...
    for component_name, component_value in component_types.items():
        ...
        if is_diffusers_model and component_name not in SINGLE_FILE_OPTIONAL_COMPONENTS:
            config_dict[component_name] = ["custom_diffusers", component_value[0].__name__]
            ...
            if is_scheduler_enum:
                ...
                config_dict[component_name] = ["custom_diffusers", "DDIMScheduler"]
    ...
    def from_single_file(cls, pretrained_model_link_or_path, **kwargs) -> Self:
        ...
        if not os.path.isdir(default_pretrained_model_config_name):
            ...
            try:
                ...
                # `local_files_only=True` but a local custom_diffusers format model config is not available in the cache
                ...
                if original_config is None:
                    ...
                    logger.warning(
                        ...
                        "To avoid this warning, please explicitly pass the `config` argument to `from_single_file` with a path to a local custom_diffusers model repo \n"
                        "e.g. `from_single_file(<my model checkpoint path>, config=<path to local custom_diffusers model repo>) \n"
