...
def _fetch_remapped_cls_from_config(config, old_class):
    ...
    # Details:
    # https://github.com/huggingface/custom_diffusers/pull/7647#discussion_r1621344818
    if remapped_class_name:
        # load custom_diffusers library to import compatible and original scheduler
...
def _fetch_index_file_legacy(
    ...
    if is_local:
        ...
        if os.path.exists(index_file):
            deprecation_message = f"This serialization format is now deprecated to standardize the serialization format between `transformers` and `custom_diffusers`. We recommend you to remove the existing files associated with the current variant ({variant}) and re-obtain them by running a `save_pretrained()`."
        ...
        if is_gguf_quant and quant_type not in SUPPORTED_GGUF_QUANT_TYPES:
            ...
            raise ValueError(
                (
                    ...
                    "\n\nTo request support for this quantization type please open an issue here: https://github.com/huggingface/custom_diffusers"
                )
            )
