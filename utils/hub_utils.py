...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    """
    ua = f"custom_diffusers/{__version__}; python/{sys.version.split()[0]}; session_id/{SESSION_ID}"
    ...
    try:
        ...
        if from_training:
            ...
            if model_description is None:
                model_description = f"This is the model card of a 🧨 custom_diffusers {component} that has been pushed on the Hub. This model card has been automatically generated."
...
def populate_model_card(model_card: ModelCard, tags: Union[str, List[str]] = None) -> ModelCard:
    ...
    if model_card.data.library_name is None:
        model_card.data.library_name = "custom_diffusers"
    ...
    if dduf_entries:
        ...
        if (
            ...
            try:
                ...
                warnings.warn(
                    f"Loading the variant {revision} from {pretrained_model_name_or_path} via `revision='{revision}'` is deprecated. Loading instead from `revision='main'` with `variant={revision}`. Loading model variants via `revision='{revision}'` will be removed in custom_diffusers v1. Please use `variant='{revision}'` instead.",
                    FutureWarning,
                )
                return model_file
            except:  # noqa: E722
                warnings.warn(
                    f"You are loading the variant {revision} from {pretrained_model_name_or_path} via `revision='{revision}'`. This behavior is deprecated and will be removed in custom_diffusers v1. One should use `variant='{revision}'` instead. However, it appears that {pretrained_model_name_or_path} currently does not have a {_add_variant(weights_name, revision)} file in the 'main' branch of {pretrained_model_name_or_path}. \n The Diffusers team and community would be very grateful if you could open an issue: https://github.com/huggingface/custom_diffusers/issues/new with the title '{pretrained_model_name_or_path} is missing {_add_variant(weights_name, revision)}' so that the correct variant file can be added.",
                    FutureWarning,
                )
        try:
            ...
            raise EnvironmentError(
                ...
                " offline mode at 'https://huggingface.co/docs/custom_diffusers/installation#offline-mode'."
            ) from e
