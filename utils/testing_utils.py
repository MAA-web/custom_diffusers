...
def nightly(test_case):
    """
    Decorator marking a test that runs nightly in the custom_diffusers CI.
...
def is_torch_compile(test_case):
    """
    Decorator marking a test that runs compile tests in the custom_diffusers CI.
...
def load_hf_numpy(path) -> np.ndarray:
    base_url = "https://huggingface.co/datasets/fusing/custom_diffusers-testing/resolve/main"
...
class CaptureLogger:
    ...
    Example:
    ```python
    >>> from custom_diffusers import logging
    >>> from custom_diffusers.testing_utils import CaptureLogger

    >>> msg = "Testing 1, 2, 3"
    >>> logging.set_verbosity_info()
    >>> logger = logging.get_logger("custom_diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.py")
