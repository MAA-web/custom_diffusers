...
def convert_state_dict_to_peft(state_dict, original_type=None, **kwargs):
    r"""
    Converts a state dict to the PEFT format The state dict can be from previous custom_diffusers format (`OLD_DIFFUSERS`), or
    new custom_diffusers format (`DIFFUSERS`). The method only supports the conversion from custom_diffusers old/new to PEFT for now.
...
def convert_state_dict_to_diffusers(state_dict, original_type=None, **kwargs):
    r"""
    Converts a state dict to new custom_diffusers format. The state dict can be from previous custom_diffusers format
    (`OLD_DIFFUSERS`), or PEFT format (`PEFT`) or new custom_diffusers format (`DIFFUSERS`). In the last case the method will
    return the state dict as is.

    The method only supports the conversion from custom_diffusers old, PEFT to custom_diffusers new for now.
...
def convert_unet_state_dict_to_peft(state_dict):
    r"""
    Converts a state dict from UNet format to custom_diffusers format - i.e. by removing some keys
    """
