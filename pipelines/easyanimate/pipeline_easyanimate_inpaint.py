...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from custom_diffusers import EasyAnimateInpaintPipeline
        >>> from custom_diffusers.pipelines.easyanimate.pipeline_easyanimate_inpaint import get_image_to_video_latent
        >>> from custom_diffusers.utils import export_to_video, load_image

        >>> pipe = EasyAnimateInpaintPipeline.from_pretrained(
        ...     "alibaba-pai/EasyAnimateV5.1-12b-zh-InP-custom_diffusers", torch_dtype=torch.bfloat16
        ... )
        >>> pipe.to("cuda")
...
class EasyAnimateInpaintPipeline(DiffusionPipeline):
    ...
    # Copied from custom_diffusers.pipelines.easyanimate.pipeline_easyanimate.EasyAnimatePipeline.encode_prompt
    def encode_prompt(
        self,
