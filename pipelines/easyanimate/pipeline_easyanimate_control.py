...
EXAMPLE_DOC_STRING = """
    Examples:
        ```python
        >>> import torch
        >>> from custom_diffusers import EasyAnimateControlPipeline
        >>> from custom_diffusers.pipelines.easyanimate.pipeline_easyanimate_control import get_video_to_video_latent
        >>> from custom_diffusers.utils import export_to_video, load_video

        >>> pipe = EasyAnimateControlPipeline.from_pretrained(
        ...     "alibaba-pai/EasyAnimateV5.1-12b-zh-Control-custom_diffusers", torch_dtype=torch.bfloat16
        ... )
        >>> pipe.to("cuda")
